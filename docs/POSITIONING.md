...
| **Discover** | Built | Home "Discover" — real market movers (`GET /api/discover`), ranked by momentum and reasoned against the owned portfolio (same player/product/sport, or "in your collection"). Catalog is no longer fixed-size (`POST /api/cards` — any real card, any sport, any era, see `apps/api/README.md` "Card catalog scope"), and Market search now also reaches a real 60,234-card bulk registry spanning thirteen sources — Topps Baseball 1952–2016, a full modern Panini Basketball release with every parallel/insert/auto, and eleven Beckett News checklist downloads covering soccer (four products), football, hockey, golf, racing, wrestling, and modern basketball/baseball (`app/checklist.py`, see "Bulk checklist" in that same doc) — still a handful of products/runs, not literally everything, and still no network/social discovery layer on top of it. |

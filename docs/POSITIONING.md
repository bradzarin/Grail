...
| **Discover** | Built | Home "Discover" — real market movers (`GET /api/discover`), ranked by momentum and reasoned against the owned portfolio (same player/product/sport, or "in your collection"). Catalog is no longer fixed-size (`POST /api/cards` — any real card, any sport, any era, see `apps/api/README.md` "Card catalog scope"), and Market search now also reaches a real 41,823-card bulk index of every Topps Baseball card 1952–2016 (`app/checklist.py`, see "Bulk checklist" in that same doc) — still just one sport/manufacturer's full run, not literally everything, and still no network/social discovery layer on top of it. |
